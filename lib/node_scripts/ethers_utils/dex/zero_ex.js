...
async function quoteAndSwap0x(
  ...
  const sellTokenDecimals = Number(sellTokenDecimalsStr);
  const buyTokenDecimals = Number(buyTokenDecimalsStr);
  
  const formattedMinOutAmount = parseFloat(minAmountOutHuman).toFixed(Number(buyTokenDecimals));
  const minAmountOutWei = ethers.parseUnits(formattedMinOutAmount, Number(buyTokenDecimals));
  ...
  // 1.5) Verify price deviation
  const quoteMinBuyAmountWei = BigInt(q.quote?.minBuyAmount || "0");
  const quoteMinBuyAmountHuman = ethers.formatUnits(quoteMinBuyAmountWei, buyTokenDecimals);
  //console.log("quote", quote);
  //console.log("minAmountOutHuman:", minAmountOutHuman);
  //console.log("quoteMinBuyAmountWei:", quoteMinBuyAmountWei.toString());
  //console.log("minAmountOutWei:", minAmountOutWei.toString());
  if (quoteMinBuyAmountWei < minAmountOutWei) {
    return {
      success: false,
      stage: "min_amount_check",
      message: "0x minBuyAmount < required minAmountOut",
      requiredMinAmountOut: minAmountOutHuman,
      requiredMinAmountOutWei: minAmountOutWei.toString(),
      quoteMinBuyAmount: quoteMinBuyAmountHuman,
      quoteMinBuyAmountWei: quoteMinBuyAmountWei.toString(),
      raw: { quote }
    };
  }
  // end check
...
function impliedPriceFromQuote(quote, sellAmountWei, sellDecimals, buyDecimals) {
  const buy  = BigInt(quote?.buyAmount ?? '0');
  const sell = BigInt(sellAmountWei ?? '0');
  if (sell === 0n || buy === 0n) return null;

  // price = (buy / 10^buyDec) / (sell / 10^sellDec)
  // => price = (buy * 10^sellDec) / (sell * 10^buyDec)
  // Scale by 1e18 to keep precision, then format back down.
  const SCALE = 10n ** 18n;
  const num = buy * (10n ** BigInt(sellDecimals)) * SCALE;
  const den = sell * (10n ** BigInt(buyDecimals));
  if (den === 0n) return null;

  const priceScaled = num / den; // still BigInt, scaled by 1e18
  return ethers.formatUnits(priceScaled, 18); // -> string like "0.000123456789..."
}
...
async function validateTokenPairWith0x(
  ...
  const sellTokenDecimals = WETH_BASE_DECIMALS;
  ...
  // Reject if 0x gave no usable data
  const missingData =
    priceSmall == null ||
    priceLarge == null ||
    impactBps == null ||
    !qSmall.quote?.buyAmount ||
    !qLarge.quote?.buyAmount ||
    qSmall.quote.buyAmount === '0' ||
    qLarge.quote.buyAmount === '0' ||
    sourcesSmall.length === 0 ||
    sourcesLarge.length === 0;

  //const accept = !(tooHighImpact || obviouslyTaxed || reverseBlocked);
  const accept = !(tooHighImpact || obviouslyTaxed || reverseBlocked || missingData);
...
async function get0xPrice(sellToken, buyToken, amount, sellTokenDecimalsStr, buyTokenDecimalsStr, apiKey) {
  try {
    ...
    if (sellAmount === 0n) {
      throw new Error('Invalid sellAmount (0) returned from 0x');
    }
    if (buyAmount === 0n) {
      throw new Error('Invalid buyAmount (0) returned from 0x');
    }
    ...
    const finalPrice = 1 / price;

    return {
      success: true,
      price: finalPrice.toFixed(18),
      sellAmountWei,
      buyAmountWei: json.buyAmount
    };
    */

    const num = Number(ethers.formatUnits(buyAmount, buyTokenDecimals));
    const den = Number(ethers.formatUnits(sellAmount, sellTokenDecimals));

    if (!Number.isFinite(num) || !Number.isFinite(den) || num === 0 || den === 0) {
      throw new Error('Non-finite or zero price from 0x (no liquidity?)');
    }
    ...
    if (!Number.isFinite(finalPrice) || finalPrice <= 0) {
      throw new Error('Non-finite final price (likely 0 liquidity)');
    }
