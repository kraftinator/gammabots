...
async function getTransactionReceipt(txHash, providerUrl) {
    ...
    try {
        ...
        // Extract amount received from logs
        const decodedLogs = receipt.logs.map(log => {
            try {
                return {
                    amountOut: ethers.formatUnits(log.data, 18) // Assuming base token has 18 decimals
                };
            } catch (error) {
                return null;
            }
        ...
        if (decodedLogs.length === 0) {
            console.error("Failed to decode transaction logs.");
            return null;
        }

        return decodedLogs[0]; // Returning first decoded log
    } catch (error) {
