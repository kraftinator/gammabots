...
async function callContractMethod(contract, methodName, args, gasPrice, nonce, { shouldWait = true } = {}) {
    try {
        //console.log('gasPrice:', gasPrice);
        //console.log('nonce:', nonce);

        const tx = await contract[methodName](...args, { gasPrice, nonce });
        if (shouldWait) {
            await tx.wait();
        }
        return tx;
...
async function infiniteApprove(privateKey, tokenAddress, providerUrl, nonce) {
    const erc20Abi = loadAbi('erc20.json');
...
async function swap(privateKey, amountIn, tokenIn, tokenOut, tokenInDecimals, feeTier, minAmountOut, providerUrl, nonce) {
    const erc20Abi = loadAbi('erc20.json');
...
/*
async function buyWithMinAmount(privateKey, amountIn, tokenIn, tokenOut, tokenInDecimals, tokenOutDecimals, feeTier, minAmountOut, providerUrl, nonce) {
    ...
    if (BigInt(quoteRaw) >= BigInt(minAmountOutRaw)) {
      ...
      const result = await swap(privateKey, amountIn, tokenIn, tokenOut, tokenInDecimals, feeTier, minAmountOutRaw, providerUrl, nonce);
      ...
      return { success: false, txHash: null, quote: formattedQuote, min_amount_out: minAmountOut, nonce: nonce };
    }
}
*/ 
...
async function sellWithMinAmount(privateKey, amountIn, tokenIn, tokenOut, tokenInDecimals, tokenOutDecimals, feeTier, minAmountOut, providerUrl, nonce) {
    // Convert minAmountOut
    const formattedMinOut = parseFloat(minAmountOut).toFixed(Number(tokenOutDecimals));
    const minAmountOutRaw = ethers.parseUnits(formattedMinOut, Number(tokenOutDecimals));
