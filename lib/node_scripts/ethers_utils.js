    ...
    try {
        ...
        */
        const tx = await wethContract.deposit({
            nonce,
            value: ethToConvert,
            maxFeePerGas,         // wei bigint
            maxPriorityFeePerGas  // wei bigint
        });
...
/**
 * Converts WETH back to ETH by calling withdraw on the WETH contract.
 * @param {string} privateKey - Private key of the wallet holding WETH
 * @param {string} providerUrl - URL for the Base network provider
 * @param {string|number} wethAmountToUnwrap - Amount of WETH to unwrap (in ETH units, not wei)
 * @param {number} nonce - Nonce for the transaction
 * @param {string} maxFeePerGasString - Max fee per gas (in wei as string)
 * @param {string} maxPriorityFeePerGasString - Max priority fee per gas (in wei as string)
 * @returns {Promise<Object>} - Transaction results
 */
async function unwrapWETH2(
  privateKey,
  providerUrl,
  wethAmountToUnwrap,
  nonce,
  ...
  try {
    // Call withdraw on WETH contract
...
async function getCurrentBlockNumber(providerUrl) {
    const provider = new ethers.JsonRpcProvider(providerUrl, CHAIN_ID);
...
// Get ETH transfer basics for a tx hash.
// Returns null if the tx isn’t found yet.
 ...
 * @param {string|number} amountEth - Amount in ETH (not wei)
 * @param {string} providerUrl - Provider URL
 * @param {number} nonce - Transaction nonce
 * @param {string} maxFeePerGasString - Max fee per gas (wei as string)
 * @param {string} maxPriorityFeePerGasString - Max priority fee per gas (wei as string)
 * @returns {Promise<Object>} - Transaction results
 */
