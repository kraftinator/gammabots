...
async function callContractMethod(contract, methodName, args, gasPrice, { shouldWait = true } = {}) {
    try {
...
async function infiniteApprove(privateKey, tokenAddress, providerUrl) {
    const erc20Abi = loadAbi('erc20.json');
    ...
    if (approvalResponse.hash) {
        return { success: true, txHash: approvalResponse.hash };
    } else {
        return { success: false, error: approvalResponse.error };
    }
}
